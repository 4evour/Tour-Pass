...
// Parameterized query helper: holds parameter strings alive while PQexecParams runs.
struct PgParams {
    std::vector<std::string> storage;

    void add(const std::string& val) { storage.push_back(val); }
    void add(int64_t val) { storage.push_back(std::to_string(val)); }
    void add(int val) { storage.push_back(std::to_string(val)); }

    const char* const* data() const {
        return storage.empty() ? nullptr : storage.data()->data() ? reinterpret_cast<const char* const*>(&storage[0]) : nullptr;
    }
};

// Workaround: std::vector<std::string>::data() gives std::string*, need const char* const*
const char* const* paramPtrs(const std::vector<std::string>& v) {
    if (v.empty()) return nullptr;
    // This is safe because std::string is contiguous and we don't modify the vector after this.
...
PGresult* execParams(PGconn* conn, const std::string& sql, const std::vector<std::string>& params) {
    int nParams = static_cast<int>(params.size());
    return PQexecParams(conn, sql.c_str(), nParams, nullptr, paramPtrs(params), nullptr, nullptr, 0);
}

PGresult* execParams(PGconn* conn, const std::string& sql) {
    return PQexecParams(conn, sql.c_str(), 0, nullptr, nullptr, nullptr, nullptr, 0);
}

std::string queryString(PGconn* conn, const std::string& sql) {
    PGresult* res = execParams(conn, sql);
    std::string val;
...
void PostgresStore::execP(const std::string& sql, const std::vector<std::string>& params) {
    if (!conn_) {
        writeFailures_++;
        throw std::runtime_error("PostgreSQL exec called with null connection");
    }
    PGresult* res = execParams(conn_, sql, params);
    if (PQresultStatus(res) != PGRES_COMMAND_OK && PQresultStatus(res) != PGRES_TUPLES_OK) {
        std::string errMsg = PQresultErrorMessage(res);
        std::cerr << "PostgreSQL exec error: " << errMsg << " [SQL: " << sql.substr(0, 120) << "]" << std::endl;
        writeFailures_++;
...
PGresult* PostgresStore::queryP(const std::string& sql, const std::vector<std::string>& params) const {
    return execParams(conn_, sql, params);
}

std::string PostgresStore::queryScalar(const std::string& sql) const {
    PGresult* res = execParams(conn_, sql);
    std::string val;
    if (PQresultStatus(res) == PGRES_TUPLES_OK && PQntuples(res) > 0 && PQgetisnull(res, 0, 0) == 0) {
        val = PQgetvalue(res, 0, 0);
    }
...
void PostgresStore::initializeSchema() {
    ...
    {
        PGresult* res = execParams(conn_, "SELECT MAX(version) FROM schema_migrations;");
        if (PQresultStatus(res) == PGRES_TUPLES_OK && PQntuples(res) > 0 && PQgetisnull(res, 0, 0) == 0) {
...
void PostgresStore::recordDataVersion(size_t poiCount, size_t edgeCount, const std::string& poisHash, const std::string& edgesHash) {
    ...
    execP("INSERT INTO data_versions(poi_count, edge_count, pois_hash, edges_hash) VALUES ($1, $2, $3, $4);",
          {std::to_string(poiCount), std::to_string(edgeCount), poisHash, edgesHash});
}
...
void PostgresStore::recordPlanningRequest(const std::string& requestId, const std::string& route, const std::string& cacheStatus, const std::string& requestJson, int responseStatus, int64_t latencyMs) {
    std::lock_guard<std::mutex> lock(mutex_);
    execP("INSERT INTO planning_requests(request_id, route, cache_status, request_json, response_status, latency_ms) VALUES ($1, $2, $3, $4, $5, $6);",
          {requestId, route, cacheStatus, requestJson, std::to_string(responseStatus), std::to_string(latencyMs)});
}
...
void PostgresStore::recordTripJob(const std::string& id, const std::string& status, const std::string& requestJson, const std::string& resultJson, const std::string& error, int64_t queueWaitMs, int64_t executionMs) {
    std::lock_guard<std::mutex> lock(mutex_);
    execP("INSERT INTO trip_jobs(id, status, request_json, result_json, error, queue_wait_ms, execution_ms) VALUES ($1, $2, $3, $4, $5, $6, $7) "
          ...
          {id, status, requestJson, resultJson, error, std::to_string(queueWaitMs), std::to_string(executionMs)});
}
...
void PostgresStore::recordBenchmarkRun(const std::string& startedAt, int durationSeconds, const std::string& concurrencyStepsJson, const std::string& summaryJson, const std::string& reportPath) {
    std::lock_guard<std::mutex> lock(mutex_);
    execP("INSERT INTO benchmark_runs(started_at, duration_seconds, concurrency_steps_json, summary_json, report_path) VALUES ($1, $2, $3, $4, $5);",
          {startedAt, std::to_string(durationSeconds), concurrencyStepsJson, summaryJson, reportPath});
}

nlohmann::json PostgresStore::recentJobs(int limit) const {
    std::lock_guard<std::mutex> lock(mutex_);
    nlohmann::json arr = nlohmann::json::array();
    PGresult* res = queryP("SELECT id, status, queue_wait_ms, execution_ms, created_at::text, updated_at::text FROM trip_jobs ORDER BY updated_at DESC LIMIT $1;",
                           {std::to_string(limit)});
...
int64_t PostgresStore::createUser(const std::string& username, const std::string& passwordHash, const std::string& role, const std::string& email, const std::string& deviceId) {
    std::lock_guard<std::mutex> lock(mutex_);
    PGresult* res = queryP("INSERT INTO users(username, password_hash, role, email, device_id) VALUES ($1, $2, $3, $4, $5) RETURNING id;",
                           {username, passwordHash, role, email, deviceId});
    if (PQresultStatus(res) != PGRES_TUPLES_OK) {
...
std::optional<UserRecord> PostgresStore::findUserByUsername(const std::string& username) {
    std::lock_guard<std::mutex> lock(mutex_);
    PGresult* res = queryP("SELECT id, username, COALESCE(email,''), password_hash, role, bonus_queries, created_at::text FROM users WHERE username = $1;",
                           {username});
...
std::optional<UserRecord> PostgresStore::findUserByEmail(const std::string& email) {
    std::lock_guard<std::mutex> lock(mutex_);
    PGresult* res = queryP("SELECT id, username, COALESCE(email,''), password_hash, role, bonus_queries, created_at::text FROM users WHERE email = $1;",
                           {email});
    auto user = extractUser(res);
    PQclear(res);
    return user;
...
std::optional<UserRecord> PostgresStore::findUserById(int64_t id) {
    std::lock_guard<std::mutex> lock(mutex_);
    PGresult* res = queryP("SELECT id, username, COALESCE(email,''), password_hash, role, bonus_queries, created_at::text FROM users WHERE id = $1;",
                           {std::to_string(id)});
    auto user = extractUser(res);
    PQclear(res);
    return user;
...
std::optional<UserRecord> PostgresStore::findUserByDeviceId(const std::string& deviceId) {
    ...
    std::lock_guard<std::mutex> lock(mutex_);
    PGresult* res = queryP("SELECT id, username, COALESCE(email,''), password_hash, role, bonus_queries, created_at::text FROM users WHERE device_id = $1 AND role = 'guest' ORDER BY created_at DESC LIMIT 1;",
                           {deviceId});
    auto user = extractUser(res);
    PQclear(res);
    return user;
...
void PostgresStore::updatePassword(int64_t userId, const std::string& newHash) {
    std::lock_guard<std::mutex> lock(mutex_);
    execP("UPDATE users SET password_hash = $1 WHERE id = $2;", {newHash, std::to_string(userId)});
}

void PostgresStore::updateRole(int64_t userId, const std::string& role) {
    std::lock_guard<std::mutex> lock(mutex_);
    execP("UPDATE users SET role = $1 WHERE id = $2;", {role, std::to_string(userId)});
}
...
void PostgresStore::storeVerificationCode(const std::string& email, const std::string& code, const std::string& purpose, int ttlSeconds) {
    std::lock_guard<std::mutex> lock(mutex_);
    execP("INSERT INTO verification_codes(email, code, purpose, expires_at) VALUES ($1, $2, $3, NOW() + make_interval(secs => $4));",
          {email, code, purpose, std::to_string(ttlSeconds)});
}
...
std::optional<std::string> PostgresStore::getValidVerificationCode(const std::string& email, const std::string& code, const std::string& purpose) {
    std::lock_guard<std::mutex> lock(mutex_);
    PGresult* res = queryP("SELECT id::text FROM verification_codes WHERE email = $1 AND code = $2 AND purpose = $3 AND used = 0 AND expires_at > NOW() ORDER BY created_at DESC LIMIT 1;",
                           {email, code, purpose});
...
void PostgresStore::markCodeUsed(int64_t codeId) {
    std::lock_guard<std::mutex> lock(mutex_);
    execP("UPDATE verification_codes SET used = 1 WHERE id = $1;", {std::to_string(codeId)});
}
...
void PostgresStore::incrementQueryCount(int64_t userId) {
    std::lock_guard<std::mutex> lock(mutex_);
    std::string today = queryScalar("SELECT CURRENT_DATE::text;");
    execP("INSERT INTO query_usage(user_id, query_date, query_count) VALUES ($1, $2, 1) "
          "ON CONFLICT(user_id, query_date) DO UPDATE SET query_count = query_usage.query_count + 1;",
          {std::to_string(userId), today});
}

void PostgresStore::addBonusQueries(int64_t userId, int amount) {
    std::lock_guard<std::mutex> lock(mutex_);
    execP("UPDATE users SET bonus_queries = bonus_queries + $1 WHERE id = $2;",
          {std::to_string(amount), std::to_string(userId)});
}

bool PostgresStore::hasEasterEggToday(int64_t userId) {
    std::lock_guard<std::mutex> lock(mutex_);
    std::string today = queryScalar("SELECT CURRENT_DATE::text;");
    PGresult* res = queryP("SELECT 1 FROM easter_egg_log WHERE user_id = $1 AND claimed_date = $2;",
                           {std::to_string(userId), today});
...
void PostgresStore::recordEasterEgg(int64_t userId) {
    std::lock_guard<std::mutex> lock(mutex_);
    std::string today = queryScalar("SELECT CURRENT_DATE::text;");
    execP("INSERT INTO easter_egg_log(user_id, claimed_date) VALUES ($1, $2) ON CONFLICT DO NOTHING;",
          {std::to_string(userId), today});
}
...
int64_t PostgresStore::saveTrip(int64_t userId, const std::string& title, const std::string& requestJson, const std::string& responseJson) {
    std::lock_guard<std::mutex> lock(mutex_);
    PGresult* res = queryP("INSERT INTO saved_trips(user_id, title, request_json, response_json) VALUES ($1, $2, $3, $4) RETURNING id;",
                           {std::to_string(userId), title, requestJson, responseJson});
...
nlohmann::json PostgresStore::listTrips(int64_t userId) {
    std::lock_guard<std::mutex> lock(mutex_);
    nlohmann::json arr = nlohmann::json::array();
    PGresult* res = queryP("SELECT id::text, title, created_at::text, COALESCE(share_id, '') FROM saved_trips WHERE user_id = $1 ORDER BY created_at DESC LIMIT 50;",
...
std::optional<nlohmann::json> PostgresStore::getTrip(int64_t tripId, int64_t userId) {
    std::lock_guard<std::mutex> lock(mutex_);
    PGresult* res = queryP("SELECT id::text, title, request_json, response_json, COALESCE(share_id, ''), created_at::text FROM saved_trips WHERE id = $1 AND user_id = $2;",
...
std::string PostgresStore::generateShareId(int64_t tripId) {
    ...
    {
        PGresult* res = queryP("SELECT COALESCE(share_id, '') FROM saved_trips WHERE id = $1;",
                               {std::to_string(tripId)});
        if (PQresultStatus(res) == PGRES_TUPLES_OK && PQntuples(res) > 0) {
            std::string existing = PQgetvalue(res, 0, 0);
            PQclear(res);
    ...
    execP("UPDATE saved_trips SET share_id = $1 WHERE id = $2 AND share_id IS NULL;",
          {shareId, std::to_string(tripId)});
    // Verify the update took effect (handle race condition)
    PGresult* check = queryP("SELECT COALESCE(share_id, '') FROM saved_trips WHERE id = $1;",
                             {std::to_string(tripId)});
    if (PQresultStatus(check) == PGRES_TUPLES_OK && PQntuples(check) > 0) {
        std::string actual = PQgetvalue(check, 0, 0);
        PQclear(check);
...
std::optional<nlohmann::json> PostgresStore::getTripByShareId(const std::string& shareId) {
    std::lock_guard<std::mutex> lock(mutex_);
    PGresult* res = queryP("SELECT id::text, title, request_json, response_json, created_at::text FROM saved_trips WHERE share_id = $1;",
                           {shareId});
    if (PQresultStatus(res) != PGRES_TUPLES_OK || PQntuples(res) == 0) { PQclear(res); return std::nullopt; }
...
void PostgresStore::submitFeedback(int64_t userId, const std::string& category, const std::string& content, const std::string& contact, const std::string& pageUrl, const std::string& userAgent) {
    std::lock_guard<std::mutex> lock(mutex_);
    execP("INSERT INTO feedback(user_id, category, content, contact, page_url, user_agent) VALUES ($1, $2, $3, $4, $5, $6);",
          {std::to_string(userId), category, content, contact, pageUrl, userAgent});
}
...
nlohmann::json PostgresStore::listFeedback(const std::string& status, int limit) {
    ...
    PGresult* res;
    if (status.empty()) {
        res = queryP("SELECT f.id::text, f.user_id::text, COALESCE(u.username, ''), f.category, f.content, f.contact, f.status, f.admin_reply, f.created_at::text FROM feedback f LEFT JOIN users u ON f.user_id = u.id ORDER BY f.created_at DESC LIMIT $1;",
                     {std::to_string(limit)});
    } else {
        res = queryP("SELECT f.id::text, f.user_id::text, COALESCE(u.username, ''), f.category, f.content, f.contact, f.status, f.admin_reply, f.created_at::text FROM feedback f LEFT JOIN users u ON f.user_id = u.id WHERE f.status = $1 ORDER BY f.created_at DESC LIMIT $2;",
...
void PostgresStore::updateFeedbackStatus(int64_t feedbackId, const std::string& status, const std::string& adminReply) {
    std::lock_guard<std::mutex> lock(mutex_);
    execP("UPDATE feedback SET status = $1, admin_reply = $2 WHERE id = $3;",
          {status, adminReply, std::to_string(feedbackId)});
}
...
nlohmann::json PostgresStore::listUsers(int limit) {
    std::lock_guard<std::mutex> lock(mutex_);
    nlohmann::json arr = nlohmann::json::array();
    PGresult* res = queryP("SELECT u.id::text, u.username, u.role, u.created_at::text, COALESCE((SELECT SUM(query_count) FROM query_usage WHERE user_id = u.id), 0)::text FROM users u ORDER BY u.created_at DESC LIMIT $1;",
                           {std::to_string(limit)});
    if (PQresultStatus(res) == PGRES_TUPLES_OK) {
        for (int i = 0; i < PQntuples(res); i++) {
...
nlohmann::json PostgresStore::queryStatsByDay(int days) {
    std::lock_guard<std::mutex> lock(mutex_);
    nlohmann::json arr = nlohmann::json::array();
    PGresult* res = queryP("SELECT query_date, SUM(query_count)::text, COUNT(DISTINCT user_id)::text FROM query_usage WHERE query_date >= (CURRENT_DATE - make_interval(days => $1))::text GROUP BY query_date ORDER BY query_date;",
                           {std::to_string(days)});
    if (PQresultStatus(res) == PGRES_TUPLES_OK) {
        for (int i = 0; i < PQntuples(res); i++) {
...
void PostgresStore::cleanupExpiredGuests(int daysRetention) {
    std::lock_guard<std::mutex> lock(mutex_);
    std::string retention = std::to_string(daysRetention);
    try {
        execP("DELETE FROM saved_trips WHERE user_id IN (SELECT id FROM users WHERE role = 'guest' AND created_at::timestamp < CURRENT_TIMESTAMP - make_interval(days => $1));", {retention});
        execP("DELETE FROM easter_egg_log WHERE user_id IN (SELECT id FROM users WHERE role = 'guest' AND created_at::timestamp < CURRENT_TIMESTAMP - make_interval(days => $1));", {retention});
        execP("DELETE FROM query_usage WHERE user_id IN (SELECT id FROM users WHERE role = 'guest' AND created_at::timestamp < CURRENT_TIMESTAMP - make_interval(days => $1));", {retention});
        execP("DELETE FROM feedback WHERE user_id IN (SELECT id FROM users WHERE role = 'guest' AND created_at::timestamp < CURRENT_TIMESTAMP - make_interval(days => $1));", {retention});
        execP("DELETE FROM users WHERE role = 'guest' AND created_at::timestamp < CURRENT_TIMESTAMP - make_interval(days => $1);", {retention});

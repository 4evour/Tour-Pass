...
当前长沙样例图规模为 `25` 个 POI 节点、`46` 条通勤边，其中包含 `1` 个酒店、`14` 个景点、`6` 个餐厅和 `4` 个夜间活动点。这个规模适合演示建模、约束解释和接口闭环，不足以证明大规模图搜索性能；大规模趋势需要使用 synthetic dataset 或真实地图数据单独评估。
...
- `shortestMinutes(from, to)`：规划热路径使用启动期预计算的 POI 两两最短通勤缓存，避免 Beam Search 评分阶段反复运行 Dijkstra。
...
在当前 `25` 节点样例图上，两者性能都足够快；保留 A* 的意义主要是展示算法岗位常见的“启发式图搜索”能力，并为未来更大 POI 图扩展做准备。面试中不要把当前样例上的 Dijkstra/A* 延迟包装成大规模性能成果。

复杂度：

- Dijkstra：`O((V + E) log V)`
- A*：最坏仍为 `O((V + E) log V)`，启发函数有效时会减少实际扩展节点数
- 两两最短通勤缓存：启动期约 `O(V * (V + E) log V)`，查询为 `O(1)`，空间为 `O(V^2)` entries。它适合当前 synthetic 规模实验和本地演示；如果扩展到几万 POI，应改成热点缓存、区域分层图或按需缓存。
...
每个时间槽先按类型、时间窗、策略标签和必去点做候选召回裁剪，再对小候选池使用完整评分函数排序并保留最多 6 个分支。规划器维护一组 Beam 状态：
...
当前参数为 `slots=5`、`beam_width=5`、`branch_factor<=6`，所以在本地样例数据上可以稳定返回。真实规模下的价值不在 25 节点最短路本身，而在“候选召回、时间窗过滤、评分拆解、多目标排序”这条可扩展规划链路。

接口会在 `days[].beam_trace` 输出调试轨迹，记录每个时间槽的输入状态数、展开状态数、保留状态数、Top 状态摘要和保留决策；保留决策会说明候选池召回/粗筛后再评分。这个字段用于面试演示和算法排查，不改变行程业务语义。
...
当前数据规模很小，但这个模块已经具备信息检索的核心表达：字段权重、词频饱和、逆文档频率和排序解释。若 POI 扩大到万级，应先构建倒排索引或接入专门检索组件，不能继续每次线性扫描全量 POI。
...
- 当前样例只有 `25 POI / 46 edges`，不能用来证明大规模图算法性能；仓库提供 `scripts/generate_synthetic_data.js` 生成 100/500/1000/5000 POI 的规模实验数据。

...
def itinerary_skeleton_output_format(expected_days: int | None = None) -> dict[str, Any]:
    ...
    day = _object(
        {
            ...
            "stops": _array(stop, minimum=1, maximum=7),
        }
    )
    hotel = _object(
        {
        ...
        "schema": _object(
            {
                "title": _string(),
                "overview": _string(),
                "highlights": _array(_string(), minimum=1, maximum=4),
                "tradeoffs": _array(_string(), minimum=1, maximum=4),
                "budget_notes": _array(
                    _described_string(
                ...
                "budget_allocation": _array(budget_category, maximum=6),
                "hotel": hotel,
                "hotels": _array(hotel, minimum=1),
                "days": _array(
                    day,
                    minimum=expected_days if expected_days is not None else 1,
                    maximum=expected_days,
                ),
            }
        ),
    }

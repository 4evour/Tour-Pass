...
### 变更内容
- `src/api.cpp` — 移除“没有管理员时自动提升下一位注册用户”的隐式规则，仅按 `TOURPASS_ADMIN_USERS` 创建管理员。
- `README.md`、`docs/api.md`、`docs/deployment.md`、`render.yaml` — 补充用户名/邮箱匹配规则与部署配置入口。
- `tests/test_reviewed_admin_policy_contract.js` — 增加管理员注册策略回归测试。

### 原因
- 防止空数据库、数据迁移或管理员记录异常时，任意新注册用户意外获得管理权限。

### 影响范围
- 影响用户名和邮箱注册的初始角色；未显式配置白名单时所有普通注册均为 `user`，已有用户角色不变。

## 2026-08-22 - 强化容器构建与双服务健康检查

### 变更内容
- `Dockerfile` — Agent 模块导入失败时终止镜像构建，健康检查同时请求 C++ API 与 Python Agent。
- `tests/test_reviewed_docker_contracts.js` — 增加构建失败语义与双服务健康检查的源码契约测试。

### 原因
- 防止依赖或模块损坏的镜像继续发布，并避免 Agent 进程退出后容器仍被平台误判为健康。

### 影响范围
- 影响 Docker 镜像构建失败条件和容器健康状态；任一服务不可用都会使健康检查失败。
...
### 变更内容
- `tools/session_store.py` — 增加会话读取、保存和受限列举接口，统一 Redis 与内存行为。
- `api_multi_agent.py` — 移除遗留 `_chat_sessions` 引用，显式持久化会话，限制多方案并发，校验 XHS 用户目录标识并卸载同步抓取。
- `graph.py` — 移除独立请求使用的无界内存检查点。
- `web/app.js` — XHS 分析请求统一发送字符串用户 ID。
- `tests/test_reviewed_agent_fixes.py` — 增加会话、并发和路径边界回归测试。

### 原因
- 修复会话端点 500、Redis 状态丢失、无界策略扇出、目录逃逸、事件循环阻塞和检查点内存增长。

### 影响范围
- 影响 Python 多 Agent 会话、行程修改、多方案规划和 XHS 解析；多方案最多四个且只接受受支持策略。
...
### 变更内容
- `README.md`、`docs/api.md` — 更新当前城市/POI/通勤边统计，修正多 Agent 启动命令和城市默认配置变量，补充数据库、队列、天气和 Agent 配置，补齐 Docker 所需的 JWT 密钥示例，并修正密码哈希实现说明。

### 原因
- README 中部分数字、环境变量、默认启动入口和安全参数已与当前源码及数据校验结果不一致。
- 保留已部署到 Render 的在线体验说明，仅同步实现细节和可复现的本地运行方式。

### 影响范围
- 仅影响项目说明文档和变更记录，不改变运行时代码、API、数据或部署配置。

## 2026-07-13 17:45 - 首屏仅保留结构化规划表单

### 变更内容 — 改了什么文件，具体改了什么
- `web/index.html` — 移除首屏自然语言模式、模式切换、快捷提示和关键词入口，默认直接展示结构化表单，并更新结果区空状态文案。
- `web/app.js` — 删除首屏自然语言规划、快捷模板及模式切换脚本，保留结构化规划和生成行程后的多轮修改逻辑。
- `web/styles.css` — 清理已移除自然语言入口、快捷模板和模式切换对应的孤儿样式。
- `tests/test_structured_form_city_selection.js` — 增加结构化表单默认可见且自然语言入口不再渲染的浏览器回归断言。
- `CHANGELOG.md` — 追加本次前端入口调整记录。

### 原因 — 为什么要改
- 用户要求移除前端自然语言规划入口，只保留结构化表单入口，同时不删除后端自然语言接口。

### 影响范围 — 改动影响了哪些功能/模块
- 影响主站 AI 智能规划首屏及其空状态展示。
- 不影响 `/agent/plan`、`/agent/chat` 等后端接口，也不影响生成行程后的多轮修改功能。

## 2026-07-06 22:35 - 修复暗色主题页面壳层适配

### 变更内容 — 改了什么文件，具体改了什么
- `web/styles.css` — 为暗色主题补齐 `--app-bg`，并把 AI 助手输入卡片的边框改为主题变量。
- `web/css/sidebar.css` — 将顶部栏、侧边栏、导航项、分隔线和底部按钮的浅色硬编码替换为主题变量，并让主内容背景跟随 `--bg`。
- `CHANGELOG.md` — 追加本次主题修复记录。

### 原因 — 为什么要改
- 用户反馈黑夜/白天切换版面存在问题；暗色模式下主内容背景仍使用浅色 `--app-bg`，侧栏/顶栏也残留浅色边框和文字，导致暗色页面没有完整适配。

### 影响范围 — 改动影响了哪些功能/模块
- 影响主站登录后的页面壳层、侧边栏、顶部栏、AI 助手输入区域在浅色/暗色主题下的视觉表现。
- 不影响行程规划接口、主题切换逻辑、后端服务、数据结构或编辑器构建产物。
...
## 2026-07-14 13:32 - 建立项目代码知识图谱

### 变更内容 — 改了什么文件，具体改了什么
- .codebase-memory/graph.db.zst — 使用 codebase-memory-mcp 对项目进行 full 模式索引并持久化知识图谱。
- CHANGELOG.md — 追加本次知识图谱建立记录。

### 原因 — 为什么要改
- 根据用户要求，先为项目建立可供后续代码发现、调用链追踪和架构分析使用的知识图谱。

### 影响范围 — 改动影响了哪些功能/模块
- 仅影响代码知识索引和变更记录，不改变项目运行时代码、配置或业务功能。

## 2026-07-14 13:43 - 增加知识图谱工作流规则

### 变更内容 — 改了什么文件，具体改了什么
- AGENTS.md — 新增项目级知识图谱规则，要求修改前读取图谱，修改并验证后重新索引并持久化。
- CHANGELOG.md — 追加本次项目规则变更记录。

### 原因 — 为什么要改
- 让后续 Codex 会话和其他 Agent 统一遵循知识图谱读取与更新流程。

### 影响范围 — 改动影响了哪些功能/模块
- 仅影响项目协作规则和知识索引维护流程，不改变运行时代码和业务功能。

## 2026-07-14 13:44 - 刷新项目知识图谱

### 变更内容 — 改了什么文件，具体改了什么
- .codebase-memory/graph.db.zst — 在新增项目规则后使用 codebase-memory-mcp 的 ast 模式重新索引并持久化知识图谱。
- CHANGELOG.md — 追加本次索引刷新记录。

### 原因 — 为什么要改
- 确保项目知识图谱与当前工作区状态保持同步。

### 影响范围 — 改动影响了哪些功能/模块
- 仅影响代码知识索引和变更记录，不改变项目运行时代码或业务功能。

## 2026-07-14 13:52 - 完善项目 Agent 规则

### 变更内容 — 改了什么文件，具体改了什么
- AGENTS.md — 补充项目目录地图、知识图谱前后置工作流、分模块验证命令、API/数据/安全边界、产物管理、Git 交付和文档沟通规则。
- CHANGELOG.md — 追加本次规则文档变更记录。

### 原因 — 为什么要改
- 参考 OpenAI Codex、Next.js 和 uv 等项目的 AGENTS.md 写法，将原先过于简略的规则改为可执行的项目协作手册。

### 影响范围 — 改动影响了哪些功能/模块
- 仅影响 Codex/Agent 的项目协作、验证和知识图谱维护流程，不改变运行时代码、接口或业务功能。

## 2026-07-14 14:00 - 下线小红书前端展示

### 变更内容 — 改了什么文件，具体改了什么
- `web/index.html`、`web/app.js` — 移除小红书侧边栏入口和前端路由展示，保留后端页面代码但不再显示。
- `tests/test_xhs_frontend_disabled.js` — 新增前端隐藏与后端接口保留契约。
- `tests/test_sidebar_hash_routing.js` — 移除已下线小红书页面的导航断言。

### 原因 — 为什么要改
- 当前纯文本解析效果不满足上线要求，截图识别的模型和成本方案尚未准备好。

### 影响范围 — 改动影响了哪些功能/模块
- 主站不再展示小红书解析板块；`/api/xhs/parse`、`/api/xhs/analyze`、`/api/xhs/proxy` 后端接口保持不变。

## 2026-07-14 14:55 - 刷新小红书下线变更后的知识图谱
...
### 原因 — 为什么要改
- 确保知识图谱反映小红书前端入口和路由已移除、后端接口仍保留的最新状态。

### 影响范围 — 改动影响了哪些功能/模块
- 仅影响代码知识索引和变更记录，不改变运行时代码或后端接口。

## 2026-07-14 15:40 - 修复行程编辑入口

### 变更内容 — 改了什么文件，具体改了什么
- `web/editor/src/NewEditorApp.tsx` — 移除 iframe 内的“返回首页”链接；通过 `tripId` 自动载入已保存行程，并支持将修改保存回原行程。
- `web/editor/src/components/Analytics/PDFExporter.tsx` — 清理模板字符串中的尾随空格，确保重新构建的发布产物通过差异检查。
- `web/app.js` — 将“我的行程”操作按钮明确为“编辑路线”。
- `web/editor-dist/` — 重新构建编辑器发布产物。
- `tests/test_editor_trip_deeplink.js` — 增加编辑入口、iframe 路由传参、保存修改和返回链接移除契约。
- `CHANGELOG.md` — 追加本次变更记录。

### 原因 — 为什么要改
- 编辑器位于主站 iframe 中，返回首页链接会把主站重新嵌入 iframe，导致页面显示异常；用户需要从“我的行程”选择具体行程后直接修改并保存。

### 影响范围 — 改动影响了哪些功能/模块
- 影响主站“我的行程”到编辑器的进入流程和编辑器顶部操作区。
- 不改变行程查询、保存更新 API 的接口契约；使用现有 `GET /trips/:id` 和 `PUT /trips/:id`。

## 2026-07-14 15:41 - 刷新行程编辑变更后的知识图谱
...
### 原因 — 为什么要改
- 确保知识图谱包含新的行程编辑入口、深链接载入和保存回写逻辑。

### 影响范围 — 改动影响了哪些功能/模块
- 仅影响代码知识索引和变更记录，不改变运行时代码或 API。

## 2026-07-15 21:12 - 优化 README 产品展示

### 变更内容 — 改了什么文件，具体改了什么
- `README.md` — 重写项目简介和用户可读的功能说明，增加线上使用流程、真实运行效果图片区、模块价值说明和 AI 规划链路。
- `docs/assets/readme/ai-planner-overview.png` — 保存 AI 规划入口截图。
- `docs/assets/readme/ai-planner-form.png` — 保存 AI 规划参数配置截图。
- `docs/assets/readme/generated-itinerary.png` — 从用户提供的屏幕录制中抽取生成行程结果画面。
- `CHANGELOG.md` — 追加本次文档和展示素材变更记录。

### 原因 — 为什么要改
- 原 README 以技术清单为主，读者难以快速理解产品用途、实际页面和使用方式。
- 使用仓库内的稳定图片素材，避免 README 依赖本机临时文件或外部图片地址。

### 影响范围 — 改动影响了哪些功能/模块
- 仅影响项目文档和 README 展示素材，不改变运行时代码、接口、部署配置或业务逻辑。

## 2026-07-15 21:54 - 增加 AI 行程保存入口

### 变更内容 — 改了什么文件，具体改了什么
- `web/app.js` — 在 AI 行程结果顶部渲染“保存到我的行程”按钮；生成新行程时初始化本次请求和结果状态；保存时复用现有 `/trips/save` 接口，并持久化保存状态。
- `web/styles.css` — 增加 AI 结果保存区的桌面端和移动端布局样式。
- `tests/test_agent_trip_save.js` — 新增浏览器回归测试，验证连续生成两次行程时可以分别保存，并检查保存请求包含对应城市和行程结果。
- `.codebase-memory/graph.db.zst` — 使用 codebase-memory-mcp fast 模式重新索引并持久化当前代码关系。
- `CHANGELOG.md` — 追加本次功能变更记录。

### 原因 — 为什么要改
- AI Agent 生成结果使用 `state.currentItinerary` 渲染，但没有接入已有保存按钮和 `state.candidates` 保存链路，导致用户无法将每次生成的结果保存到“我的行程”。
- 原保存冷却时间会跨行程保留，新行程生成完成后需要重置，确保每个新结果都能立即保存。
